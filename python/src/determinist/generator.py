...
def version():
    version_number = f"{version_tuple[0]}.{version_tuple[1]}.{version_tuple[2]}"
    commit_hash, date = version_tuple[4].split('.')
    commit_hash = commit_hash[1:]
    date = date[1:]
    date_obj = datetime.strptime(str(date), "%Y%m%d")
    build_date = date_obj.strftime("%B %d, %Y")
    python_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"

    version_table = Table(box=box.MINIMAL_DOUBLE_HEAD)
    version_table.add_column(f"\nDeterminist [bold cyan]{version_number}[/]", justify="center")
    version_table.add_row(f"Commit: [bold cyan]{commit_hash}[/]")
    version_table.add_row(f"Built: [bold cyan]{build_date}[/]")
